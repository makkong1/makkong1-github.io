...
§1의 재구성 헬퍼(`getMyConversationsBefore()`)는 `findByConversationIdxAndUserIdx`와 `findByConversationIdxAndStatus`만 재현했는데, **실제 커밋에는 참여자 조회가 한 번 더(다른 컬럼 세트로) 발생하고 있었다** — 아마 `ConversationConverter.toDTO()`가 `conversation.getParticipants()`도 별도로 lazy 접근했을 가능성이 높다.

**결론**: 개선 효과는 §1이 보여준 것보다 **실제로는 더 컸다**(41→4, -90.2% vs 재구성의 21→4, -80.95%). 재구성이 실제 역사를 과소평가하는 방향으로 어긋난 사례 — 이번 재검증이 아니었으면 몰랐을 발견이다.

**추가 점검(2026-07-12, 후속)**: 위 추정이 현재 dev 코드에도 남아있는 문제인지 확인했다. 현재 `ConversationConverter.toDTO()`(`backend/main/.../chat/converter/ConversationConverter.java`)를 읽어보면:
...
`getParticipants()` 호출 자체가 없다 — 이미 완전히 제거돼 있다. 496e121a 시점의 "정체불명 10개 쿼리"는 그 시절 얘기고, **현재 dev 코드엔 재발 우려가 없다.** 추가 조치 불필요.

...
sequenceDiagram
  ...
  W->>S: sendMessage()
  S->>DB: sender 제재 상태, conversation, participant 검증
  S->>DB: ChatMessage 저장
...
Chat 도메인은 제재 사용자가 이미 채팅방 참여자인 경우를 고려해 조회와 상태 변경을 분리한다.

| 흐름 | 처리 |
|---|---|
| REST/STOMP 메시지 전송 | `ChatMessageService.sendMessage()`에서 sender가 `isSanctioned()`면 403 |
| 채팅방 목록/상세 | ACTIVE 참여자 중 제재 사용자가 있으면 `ConversationDTO.hasSanctionedParticipant=true` |
| Care 거래 확정 | 현재 사용자, ACTIVE 참여자 전체, 실제 requester/provider 중 제재자가 있으면 거래 확정 차단 |
| Meetup 참가 취소 연동 | Meetup 도메인의 제재 후속 처리에서 채팅방 참여자를 `LEFT`로 전환 시도 |

기존 메시지 이력은 삭제하지 않는다. 1차 구현에서는 메시지 본문 마스킹도 하지 않고, 상대방 UI가 DTO 안내 플래그를 기반으로 “제재된 사용자” 상태를 표시한다.

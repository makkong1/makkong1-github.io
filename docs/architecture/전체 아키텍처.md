...
### 4. 위치 기반 서비스
- **기능**: 공공데이터 기반 장소 정보, 리뷰 시스템, 키워드·반경·지역 검색, AI 추천(`GET /api/location-services/recommend`) 등
- **특징**: CSV 배치 임포트, 하이브리드 검색, Spring Cache·Redis, 평점(리뷰) 반영
- **로드맵**: 장소 “추천” 기능은 검증 후 **Pet Data API + `GET /api/recommend`**로 단일화하고, JVM 내 `LocationRecommendAgentService`/위 엔드포인트는 **제거 예정**(중복). 당분간 유지한다. 상세는 [Recommendation 도메인 §1.4](../domains/recommendation.md).

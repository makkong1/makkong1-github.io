...
graph TB
    ...
    subgraph "Application Layer (단일 Spring Boot)"
        SECURITY[Spring Security<br/>JWT 인증/인가]
        CONTROLLERS[REST / WebSocket(STOMP)<br/>SSE 엔드포인트]
    end
    ...
    subgraph "Service Layer"
        ...
        PET_RECOMMEND_SVC[PetRecommendation Service<br/>의도 signal, 주변서비스 추천]
    ...
    subgraph "NLP Layer"
        PET_NLP[Python FastAPI NLP Server<br/>/api/pet-intent/analyze]
    end

    subgraph "External Services"
        NAVER_MAP[네이버 클라우드 Maps API<br/>Geocoding, Directions 등]
        EMAIL[Email Service<br/>SMTP]
        OAUTH[OAuth2 Providers<br/>Google, Naver 등<br/>(등록·라우팅은 설정 기준)]
    end

    subgraph "Scheduler Layer"
        STATS_SCHED[StatisticsScheduler<br/>일별 통계 집계]
        POPULAR_SCHED[BoardPopularityScheduler<br/>인기글 스냅샷]
        SANCTION_SCHED[UserSanctionScheduler<br/>제재 자동 해제]
        CARE_SCHED[CareRequestScheduler<br/>케어 요청 만료 처리]
    end
    ...
    LOCATION_SVC --> REPOS
    LOCATION_SVC --> PET_RECOMMEND_SVC
    PET_RECOMMEND_SVC --> LOCATION_SVC
    PET_RECOMMEND_SVC --> PET_NLP
    PET_RECOMMEND_SVC --> REPOS
    PET_RECOMMEND_SVC --> REDIS
    MEETUP_SVC --> REPOS
    MEETUP_SVC --> CHAT_SVC
    MISSING_SVC --> REPOS
    CHAT_SVC --> REPOS
    ...
    style PET_RECOMMEND_SVC fill:#e8f5e9
    style PET_NLP fill:#e8f5e9
...
- **기능**: 펫코인 충전/차감, 에스크로, Care/Chat 거래 연동
- **특징**: 비관적 락으로 Race Condition 방지, 거래 내역 DB 페이징
- **상세 문서**: [Payment 도메인](../domains/payment.md), [펫케어 코인 관련 흐름](./펫케어%20코인%20관련%20흐름.md)

### 4. 위치 기반 서비스

- **기능**: 공공데이터 기반 장소 정보, 리뷰 시스템, 키워드·반경·지역 검색
- **특징**: CSV 배치 임포트, 하이브리드 검색, Spring Cache·Redis, 평점(리뷰) 반영
...
- **기능**: Python NLP 서버로 사용자 텍스트 의도 분석, 추천 카테고리/signal 저장, 주변서비스 추천 카드 및 점수 기반 장소 추천
- **특징**: Spring은 DB 기반 장소 조회와 점수 계산을 담당하고, Python FastAPI 서버는 의도 분류·urgency 판정·태그 추출만 담당. domain×urgency별 confidence threshold(MEDICAL+HIGH→0.55, MEDICAL→0.65, FOOD_SNACK 계열→0.45) 및 TTL(MEDICAL+HIGH→1일, MEDICAL→3일, LODGING_TRAVEL→14일, default→7일)로 domain별 signal 품질·신선도 관리. 추천 카드 상호작용은 `SignalInteractionLog`(CLICKED/DISMISSED/CONVERTED)에 기록
- **상세 문서**: [Recommendation 도메인](../domains/recommendation.md)
...
- **OAuth2 소셜 로그인**: 코드상 사용자 로딩은 주로 **Google·Naver** (`OAuth2UserProviderRouter`). 추가 프로바이더는 `application` OAuth2 등록에 따름.
...
- **연동 방식**: `AttachmentFileService` 등으로 타겟 타입별 첨부·동기화
...
- **Map API**: 네이버 클라우드 플랫폼 Maps (Geocoding, Reverse Geocoding, Directions 등, `maps.apigw.ntruss.com`)
- **OAuth2**: Google, Naver (백엔드 라우팅 기준; 기타는 클라이언트 등록에 따름)
...
3. [Payment 도메인 (펫코인)](../domains/payment.md)
4. [위치 기반 서비스 아키텍처](./위치%20기반%20서비스%20아키텍처.md)
5. [산책 & 오프라인 모임 아키텍처](./산책%20&%20오프라인%20모임%20아키텍처.md)
6. [채팅 시스템 아키텍처](./채팅%20시스템%20설계.md)
7. [알림 시스템 아키텍처](./알림%20시스템%20아키텍처.md)
8. [관리자 대시보드 & 통계 시스템 아키텍처](./관리자%20대시보드%20&%20통계%20시스템%20아키텍처.md)
9. [신고 및 제재 시스템 아키텍처](./신고%20및%20제재%20시스템%20아키텍처.md)

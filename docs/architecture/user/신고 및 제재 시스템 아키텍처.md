...
#### 신고 생성
요청은 **record** `ReportRequestDTO(targetType, targetIdx, reporterId, reason)`이며, 검증 실패 시 `ReportValidationException` / `ReportConflictException` / `ReportTargetNotFoundException` 등 도메인 예외를 사용합니다.
...
public ReportDTO handleReport(Long reportId, Long adminUserId, ReportHandleRequest req) {
    ...
    if (req.getActionTaken() != null &&
        ...
        userSanctionService.applySanctionFromReport(
                report.getTargetIdx(), // 신고 대상 idx (아래 "구현 주의" 참고)
                req.getActionTaken(),
                sanctionReason,
                admin.getIdx(),
                reportId);
...
**구현 주의 (`applySanctionFromReport`의 첫 인자)**  
`UserSanctionService.applySanctionFromReport(Long userId, ...)`에 **`report.getTargetIdx()`가 그대로 전달**됩니다. `PET_CARE_PROVIDER`는 신고 대상이 사용자이므로 idx가 일치하지만, `BOARD` / `COMMENT` / `MISSING_PET`은 **콘텐츠(게시글·댓글 등)의 ID**이므로 제재를 “작성자 사용자”로 매핑하려면 별도 조회·보완이 필요합니다. (현재 코드 기준)
...
- `applySanctionFromReport()`: 신고 처리 시 자동 제재 적용 (`WARN_USER` → 경고, `SUSPEND_USER` → **영구 차단 `addBan`** — 일시 정지가 아님)
...
이름은 `SUSPEND_USER`이나 **구현은 `addBan`(영구 차단)** 입니다. 일시 이용제한(`SUSPENSION`)이 아닙니다.
...
public void applySanctionFromReport(Long userId, ReportActionType actionType, String reason, Long adminId,
    ...
    switch (actionType) {
        ...
        case SUSPEND_USER -> addBan(userId, reason, adminId, reportId);
        default -> { }
    }
}
```
...
sequenceDiagram
    ...
    User->>RC: POST /api/reports<br/>(targetType, targetIdx, reporterId, reason)
    RC->>RS: createReport(request)
    
    RS->>RS: 입력 검증<br/>(targetType, targetIdx, reporterId, reason)
...
**신고 처리에서의 조치 타입**: `WARN_USER`는 경고 누적 로직을 태우고, `SUSPEND_USER`는 서비스상 **`addBan`(영구 차단)** 으로 처리됨(이름과 구현이 다를 수 있음).
...
- **미리보기**: `buildTargetPreview`는 **우선 `commentRepository.findById`만** 조회합니다. 실종 댓글만 존재하는 ID인 경우 미리보기가 `(삭제됨)`에 가깝게 나올 수 있음(구현 한계).
...
Body: ReportRequestDTO { targetType, targetIdx, reporterId, reason }
→ ReportDTO
```
- `reporterId`는 **`Users`의 기본키(idx)** 를 요청 본문에 포함해야 함(클라이언트에서 로그인 사용자와 일치시키는 것이 일반적).
...
- `WARN_USER` / `SUSPEND_USER` 시 `UserSanctionService.applySanctionFromReport` 호출 (**`SUSPEND_USER` → 영구 차단**)
...
- **신고 처리 연동**: `WARN_USER` → 경고 / `SUSPEND_USER` → **영구 차단** (`addBan`)
- **자동 해제**: 스케줄러를 통한 만료된 이용제한 자동 해제

### 3. 제재 수단 (서비스 / 신고 연동)
- **경고**: `addWarning` — 누적 시 자동 3일 이용제한
- **일시 이용제한**: `addSuspension` — 주로 경고 3회 도달 시 자동 호출
- **영구 차단**: `addBan` — 신고 처리에서 `SUSPEND_USER` 선택 시 호출 (이름과 달리 일시 정지가 아님)

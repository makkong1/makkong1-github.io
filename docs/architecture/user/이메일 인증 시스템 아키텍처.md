...
```java
// 실제 코드: domain/user/entity/EmailVerificationPurpose.java
public enum EmailVerificationPurpose {
    REGISTRATION,        // 회원가입(가입 전·가입 직후 인증 메일)
    PASSWORD_RESET,      // 비밀번호 재설정 링크(비밀번호 찾기 메일)
    PET_CARE,
    MEETUP,
    LOCATION_REVIEW,
    BOARD_EDIT,
...
- ✅ 모임 생성/참여
- ✅ 펫케어 요청/지원
- ✅ 비밀번호 변경 (로그인 상태에서 `PATCH /api/users/me/password` — 서버에서 `emailVerified` 검사)

---

## 📡 API 엔드포인트

### 이메일 인증 메일 발송 (로그인 사용자)
...
- `purpose`는 위 enum 이름과 동일한 문자열(대소문자 무관, 서버에서 `valueOf`).
...
```http
POST /api/users/email/verify/pre-registration
Content-Type: application/json

{ "email": "user@example.com" }
```

- 가입 전에 이메일 소유 확인용. 개발 프로필에서는 `app.email-verification.skip-in-dev=true`일 때 메일 대신 Redis에 완료 상태를 바로 쓸 수 있음(`EmailVerificationService`).

```http
GET /api/users/email/verify/pre-registration/check?email=user@example.com
```

- 회원가입 폼에서 위 Redis(또는 검증 완료) 여부 확인.
...
- `EmailVerificationService.sendPasswordResetEmail` → `PASSWORD_RESET` 용도의 메일 발송.
...
**설명**: 
- 브라우저/클라이언트가 이 URL을 호출합니다.
- 토큰 검증 후 기존 사용자면 `emailVerified = true` 저장(용도와 무관하게 단일 플래그).
- **HTTP 302가 아니라 JSON**으로 `success`, `message`, `purpose`, 필요 시 `email`, **`redirectUrl`** 을 반환합니다. 프론트는 `redirectUrl`로 라우팅하면 됩니다(`UserProfileController`).
- **참고**: 회원가입 전용 토큰(`isPreRegistration` 등)은 응답에 `register?emailVerified=…` 형태가 붙는 분기가 있고, 그 외에는 `getRedirectUrl(purpose)`로 프론트 경로(`/password-reset`, `/care-requests`, `/meetups` 등)가 내려갑니다. 세부 분기는 `UserProfileController.verifyEmail`을 보면 됩니다.
...
- 키 prefix: `email_verification:pre_registration:` + 이메일, 값 `verified`, TTL **24시간** (`EmailVerificationService` 상수).
- 가입 완료 후 `removePreRegistrationVerification` 등으로 정리 가능.

### 토큰 생성 및 검증

```java
// JwtUtil — 이메일 인증 JWT 만료: 24시간 (EMAIL_VERIFICATION_TOKEN_EXPIRE_TIME)

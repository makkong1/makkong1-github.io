...
3. **데이터 병합** (Redis에 데이터가 있는 경우)
   - **DB에서 전체 알림 조회**: 사용자의 모든 알림 조회 (최신순)
...
2. **DB 카운트 쿼리**
   - `countUnreadByUser()` 메서드로 읽지 않은 알림 개수만 조회
...
2. **읽지 않은 알림 조회**
   - `isRead = false`인 모든 알림 조회

3. **일괄 읽음 처리**
   - 모든 알림의 `isRead = true` 설정
   - DB 일괄 저장 (`saveAll()`)

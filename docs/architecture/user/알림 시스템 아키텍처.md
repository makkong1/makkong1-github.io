# 알림 시스템 아키텍처

## 📋 개요

알림 시스템은 Petory 서비스의 다양한 도메인 이벤트(댓글 작성 등)를 사용자에게 실시간으로 전달하는 핵심 기능입니다. **이중 저장소 전략**(MySQL 영구 저장 + Redis 최신 목록 캐시)과 **SSE(Server-Sent Events)**로 푸시를 제공합니다. SSE는 `EventSource`가 커스텀 헤더를 쓰기 어려워 **`JwtAuthenticationFilter`가 `Authorization: Bearer` 외에 쿼리 파라미터 `token`으로도 JWT를 받도록** 되어 있습니다.

## 🏗️ 시스템 아키텍처

### 전체 구조도

```mermaid
graph TB
    subgraph "Frontend"
        NAV[Navigation<br/>SSE 클라이언트]
        NOTIF[Notification UI<br/>알림 표시]
    end
    
    subgraph "Backend API Layer"
        NC[NotificationController<br/>/api/notifications]
        SSE[SSE Endpoint<br/>/api/notifications/stream]
    end
    
    subgraph "Service Layer"
        NS[NotificationService<br/>알림 생성, 조회, 병합]
        NSS[NotificationSseService<br/>SSE 연결 관리]
    end
    
    subgraph "Data Layer"
        NR[NotificationRepository]
        MySQL[(MySQL<br/>notifications 테이블)]
        Redis[(Redis<br/>notification:userId)]
    end
    
    subgraph "Domain Triggers"
        CRCS[CareRequestCommentService<br/>케어 요청 댓글]
        CS[CommentService<br/>커뮤니티 댓글]
        MPCS[MissingPetCommentService<br/>실종 제보 댓글]
    end
    
    NAV --> SSE
    NAV --> NC
    NOTIF --> NC
    
    SSE --> NSS
    NC --> NS
    
    NS --> NR
    NS --> Redis
    NS --> NSS
    
    NSS --> NAV
    
    CRCS --> NS
    CS --> NS
    MPCS --> NS
    
    NR --> MySQL
    
    style NS fill:#e1f5ff
    style NSS fill:#fff4e1
    style Redis fill:#ffe1f5
```

## 🔧 핵심 컴포넌트

### 1. NotificationService (알림 서비스)

**역할**: 알림 생성, 조회, Redis와 MySQL 병합, 읽음 처리

**주요 메서드**:
- `createNotification()`: 알림 생성 및 발송 (DB 저장, Redis 저장, SSE 전송)
- `getUserNotifications()`: 사용자 알림 목록 조회 (Redis + DB 병합)
- `getUnreadNotifications()`: 읽지 않은 알림 목록 조회
- `getUnreadCount()`: 읽지 않은 알림 개수 조회
- `markAsRead()`: 알림 읽음 처리
- `markAllAsRead()`: 모든 알림 읽음 처리

**핵심 로직**:

#### 알림 생성 및 발송
```java
@Transactional
public NotificationDTO createNotification(Long userId, NotificationType type, 
        String title, String content, Long relatedId, String relatedType) {
    Users user = usersRepository.findById(userId)
        .orElseThrow(UserNotFoundException::new);

    // 1. 알림 엔티티 생성
    Notification notification = Notification.builder()
        .user(user)
        .type(type)
        .title(title)
        .content(content)
        .relatedId(relatedId)
        .relatedType(relatedType)
        .isRead(false)
        .build();
    
    // 2. MySQL에 저장 (영구 저장)
    Notification saved = notificationRepository.save(notification);
    NotificationDTO dto = notificationConverter.toDTO(saved);
    
    // 3. Redis에 실시간 알림 저장 (최신 알림 목록 관리)
    saveToRedis(userId, dto);
    
    // 4. SSE를 통해 실시간 알림 전송 (연결된 경우)
    sseService.sendNotification(userId, dto);
    
    return dto;
}
```

#### Redis 저장 로직
```java
private void saveToRedis(Long userId, NotificationDTO notification) {
    String redisKey = REDIS_KEY_PREFIX + userId; // "notification:{userId}"
    List<NotificationDTO> existingNotifications = getFromRedis(userId);
    
    // 수정 가능한 리스트 생성
    List<NotificationDTO> notifications;
    if (existingNotifications == null || existingNotifications.isEmpty()) {
        notifications = new ArrayList<>();
    } else {
        notifications = new ArrayList<>(existingNotifications);
    }
    
    // 최신 알림을 맨 앞에 추가 (최대 50개만 유지)
    notifications.add(0, notification);
    if (notifications.size() > 50) {
        notifications = notifications.subList(0, 50);
        notifications = new ArrayList<>(notifications);
    }
    
    // Redis에 저장 (TTL: 24시간)
    notificationRedisTemplate.opsForValue().set(redisKey, notifications,
        Duration.ofHours(REDIS_TTL_HOURS));
}
```

#### 알림 조회 및 병합
```java
public List<NotificationDTO> getUserNotifications(Long userId) {
    // 1. Redis에서 먼저 조회 시도
    List<NotificationDTO> redisNotifications = getFromRedis(userId);
    
    if (redisNotifications != null && !redisNotifications.isEmpty()) {
        // 2. DB에서 전체 알림 조회
        List<NotificationDTO> dbNotifications = notificationRepository
            .findByUserIdOrderByCreatedAtDesc(userId)
            .stream()
            .map(notificationConverter::toDTO)
            .collect(Collectors.toList());
        
        // 3. Redis와 DB 데이터 병합 (중복 제거)
        return mergeNotifications(redisNotifications, dbNotifications);
    }
    
    // Redis에 없으면 DB에서만 조회
    return notificationRepository.findByUserIdOrderByCreatedAtDesc(userId)
        .stream()
        .map(notificationConverter::toDTO)
        .collect(Collectors.toList());
}
```

#### 병합 전략 (중복 제거 및 정렬)
```java
private List<NotificationDTO> mergeNotifications(
        List<NotificationDTO> redisNotifications,
        List<NotificationDTO> dbNotifications) {
    
    // Redis 알림 ID 집합 생성
    Set<Long> redisIds = redisNotifications.stream()
        .map(NotificationDTO::getIdx)
        .collect(Collectors.toSet());
    
    // Redis에 없는 DB 알림만 추가
    List<NotificationDTO> merged = new ArrayList<>(redisNotifications);
    dbNotifications.stream()
        .filter(n -> !redisIds.contains(n.getIdx()))
        .forEach(merged::add);
    
    // 최신순 정렬 (createdAt 기준 내림차순)
    merged.sort((a, b) -> {
        if (a.getCreatedAt() == null && b.getCreatedAt() == null) return 0;
        if (a.getCreatedAt() == null) return 1;
        if (b.getCreatedAt() == null) return -1;
        return b.getCreatedAt().compareTo(a.getCreatedAt());
    });
    
    return merged;
}
```

### 2. NotificationSseService (SSE 서비스)

**역할**: SSE 연결 관리 및 실시간 알림 전송

**주요 메서드**:
- `createConnection()`: 사용자별 SSE 연결 생성
- `sendNotification()`: 특정 사용자에게 알림 전송
- `removeConnection()`: 사용자 연결 해제(명시적 종료)
- `getConnectedUserCount()`: 현재 연결 수(모니터링용)

**연결 저장 방식**: `ConcurrentHashMap<Long, SseEmitter>`에 **`userId`당 하나의 emitter만** 유지한다. 동일 사용자가 새로 `GET /stream`을 열면 `put`으로 **이전 연결이 덮어써지며**, 예전 탭·기기 쪽은 더 이상 `sendNotification` 대상이 아니다(멀티 탭 동시 수신이 필요하면 구조 변경 필요).

**핵심 로직**:

#### SSE 연결 생성
```java
public SseEmitter createConnection(Long userId) {
    SseEmitter emitter = new SseEmitter(3600000L); // 1시간 타임아웃
    
    emitter.onCompletion(() -> {
        log.info("SSE 연결 완료: userId={}", userId);
        emitters.remove(userId);
    });
    
    emitter.onTimeout(() -> {
        log.info("SSE 연결 타임아웃: userId={}", userId);
        emitters.remove(userId);
    });
    
    emitter.onError((ex) -> {
        log.error("SSE 연결 오류: userId={}, error={}", userId, ex.getMessage());
        emitters.remove(userId);
    });
    
    emitters.put(userId, emitter);
    log.info("SSE 연결 생성: userId={}, 현재 연결 수={}", userId, emitters.size());
    
    return emitter;
}
```

#### 실시간 알림 전송
```java
public void sendNotification(Long userId, NotificationDTO notification) {
    SseEmitter emitter = emitters.get(userId);
    if (emitter != null) {
        try {
            emitter.send(SseEmitter.event()
                .name("notification")
                .data(notification));
            log.info("알림 전송 성공: userId={}, notificationId={}", userId, notification.getIdx());
        } catch (IOException e) {
            log.error("알림 전송 실패: userId={}, error={}", userId, e.getMessage());
            emitters.remove(userId);
            try {
                emitter.completeWithError(e);
            } catch (Exception ex) {
                log.error("emitter 완료 처리 실패", ex);
            }
        }
    } else {
        log.debug("SSE 연결 없음: userId={} (알림은 DB/Redis에 저장됨)", userId);
    }
}
```

### 3. Notification 엔티티

**역할**: 알림 데이터를 저장하는 엔티티

**주요 필드**:
- `idx`: 알림 ID
- `user`: 알림을 받을 사용자
- `type`: 알림 타입 (`CARE_REQUEST_COMMENT`, `BOARD_COMMENT`, `MISSING_PET_COMMENT`)
- `title`: 알림 제목
- `content`: 알림 내용 (최대 500자)
- `relatedId`: 관련 **상위 리소스** ID (구현상 댓글 알림도 `BOARD`/`CARE_REQUEST`/`MISSING_PET`의 **글·요청·제보 idx**를 넣음 — 댓글 idx가 아님)
- `relatedType`: 관련 타입 문자열 (`BOARD`, `CARE_REQUEST`, `MISSING_PET` 등)
- `isRead`: 읽음 여부
- `createdAt`: 생성 시간

**엔티티 구조**:
```java
@Entity
@Table(name = "notifications")
public class Notification {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long idx;
    
    @ManyToOne
    @JoinColumn(name = "user_idx", nullable = false)
    private Users user;
    
    @Enumerated(EnumType.STRING)
    @Column(nullable = false)
    private NotificationType type;
    
    @Column(nullable = false)
    private String title;
    
    @Column(length = 500)
    private String content;
    
    @Column(name = "related_id")
    private Long relatedId;
    
    @Column(name = "related_type")
    private String relatedType;
    
    @Column(name = "is_read", nullable = false)
    @Builder.Default
    private Boolean isRead = false;
    
    @Column(name = "created_at")
    private LocalDateTime createdAt;

    @PrePersist
    protected void onCreate() {
        this.createdAt = LocalDateTime.now();
    }
}
```

### 4. NotificationType (알림 타입 Enum)

**알림 타입**:
- `CARE_REQUEST_COMMENT`: 펫케어 요청글 댓글 알림
- `BOARD_COMMENT`: 커뮤니티 게시글 댓글 알림
- `MISSING_PET_COMMENT`: 실종 제보 게시글 댓글 알림

## 🔄 비즈니스 로직 흐름

### 1. 알림 생성 및 발송 흐름

**단계별 처리 과정** (`NotificationService.createNotification()`):

1. **사용자 확인**
   - 알림을 받을 사용자 정보 조회
   - 사용자가 존재하지 않으면 예외 발생

2. **알림 엔티티 생성**
   - `Notification` 엔티티 생성
   - 알림 타입, 제목, 내용, 관련 ID/타입, 읽음 상태(`false`) 설정
   - DB 저장

3. **DTO 변환**
   - 저장된 알림을 DTO로 변환

4. **Redis에 저장** (`saveToRedis()`)
   - Redis Key: `notification:{userId}`
   - 기존 알림 목록 조회
   - 최신 알림을 맨 앞에 추가 (최대 50개만 유지)
   - TTL: 24시간으로 설정하여 저장

5. **SSE 실시간 알림 전송** (`sseService.sendNotification()`)
   - 사용자가 SSE 연결되어 있으면 실시간 전송
   - 연결되어 있지 않으면 알림은 DB/Redis에만 저장 (로그만 기록)

6. **응답 반환**
   - 생성된 알림 DTO 반환

**특징:**
- MySQL과 Redis 이중 저장 (안정성 + 성능)
- 최신 알림 50개만 Redis에 저장 (메모리 효율)
- SSE 연결이 없어도 알림은 저장됨 (나중에 조회 가능)

### 2. 알림 조회 및 병합 흐름

**단계별 처리 과정** (`NotificationService.getUserNotifications()`):

1. **Redis에서 조회**
   - Redis Key: `notification:{userId}`로 조회
   - 기존 알림 목록 반환 (또는 null)

2. **데이터 병합** (Redis에 데이터가 있는 경우)
   - **DB에서 전체 알림 조회**: `userId`로 사용자의 모든 알림 조회 (최신순)
   - `Users` 엔티티는 별도로 조회하지 않음
   - **병합 처리** (`mergeNotifications()`):
     - Redis 알림 ID Set 생성 (중복 제거용)
     - DB 알림 중 Redis에 없는 알림만 추가
     - 최신순 정렬 (`createdAt` 기준 내림차순)
   - 병합된 알림 목록 반환

4. **DB만 조회** (Redis에 데이터가 없는 경우)
   - DB에서 사용자의 모든 알림 조회 (최신순)
   - DTO로 변환하여 반환

**병합 전략:**
- Set 기반 중복 제거 (O(1) 조회 성능)
- Redis와 DB 데이터 통합하여 완전한 데이터 제공
- 최신순 정렬로 일관된 사용자 경험

**특징:**
- Redis 캐싱이 있으면 빠른 조회 성능
- Redis 캐싱이 없어도 DB에서 조회하여 안정성 확보

### 3. 읽지 않은 알림 조회 흐름

**읽지 않은 알림 목록 조회** (`NotificationService.getUnreadNotifications()`):

1. **DB 조회**
   - `userId + isRead = false` 조건으로 알림 조회 (최신순)
   - `Users` 엔티티를 별도로 조회하지 않음
   - DTO로 변환하여 반환

**읽지 않은 알림 개수 조회** (`NotificationService.getUnreadCount()`):

1. **DB 카운트 쿼리**
   - `countUnreadByUserId()`로 사용자 ID 기준 읽지 않은 알림 개수만 조회
   - `Users` 엔티티를 별도로 조회하지 않음
   - 트랜잭션 없이 실행 (`NOT_SUPPORTED`) - SSE 연결 등 장시간 연결에서 호출 가능

**특징:**
- 읽지 않은 알림만 필터링하여 조회
- 카운트 쿼리로 성능 최적화
- 트랜잭션 없이 실행하여 연결 유지

### 4. 알림 읽음 처리 흐름

**단일 알림 읽음 처리** (`NotificationService.markAsRead()`):

1. **알림 확인**
   - 알림 존재 여부 확인

2. **권한 확인**
   - 본인의 알림인지 확인 (`notification.getUser().getIdx() == userId`)
   - 본인의 알림이 아니면 예외 발생

3. **읽음 처리**
   - `isRead = true` 설정
   - DB 저장

4. **Redis에서 제거** (`removeFromRedis()`)
   - Redis Key: `notification:{userId}`로 기존 알림 목록 조회
   - 해당 알림 ID를 리스트에서 제거
   - 수정된 리스트를 Redis에 저장 (TTL 유지)

**모든 알림 읽음 처리** (`NotificationService.markAllAsRead()`):

1. **JPQL bulk UPDATE**
   - `userId + isRead = false` 조건으로 `isRead = true`
   - 미읽음 알림 엔티티를 메모리에 로딩하지 않음
   - 알림 개수와 관계없이 UPDATE statement 1회
   - `clearAutomatically`, `flushAutomatically`로 영속성 컨텍스트 불일치 방지

2. **Redis에서 전체 제거**
   - Redis Key: `notification:{userId}` 삭제
   - 해당 사용자의 Redis 알림 목록 완전 제거

**특징:**
- 본인의 알림만 읽음 처리 가능 (권한 체크)
- Redis와 DB 동기화 (읽음 처리 시 Redis에서도 제거)
- 전체 읽음 100개 기준 MySQL/Hibernate Statistics 실측 `102 → 1 prepared statements`

### 5. SSE 연결 생성 흐름

**단계별 처리 과정** (`NotificationSseService.createConnection()`):

1. **SseEmitter 생성**
   - 타임아웃: 1시간 (3600000ms)
   - 단방향 통신 (서버 → 클라이언트)

2. **이벤트 리스너 등록**
   - **onCompletion**: 연결 완료 시 자동 호출
     - `emitters`에서 사용자 제거
     - 로그 기록
   - **onTimeout**: 타임아웃 시 자동 호출
     - `emitters`에서 사용자 제거
     - 로그 기록
   - **onError**: 오류 발생 시 자동 호출
     - `emitters`에서 사용자 제거
     - 로그 기록

3. **연결 저장**
   - `ConcurrentHashMap`에 사용자 ID와 Emitter 매핑 저장
   - 동시성 안전 (여러 스레드에서 안전하게 접근)

4. **응답 반환**
   - 생성된 `SseEmitter` 반환

**특징:**
- 자동 정리 (완료/타임아웃/오류 시 자동으로 `emitters`에서 제거)
- `ConcurrentHashMap`으로 동시성 안전
- 타임아웃 설정으로 리소스 관리

### 6. SSE 실시간 알림 전송 흐름

**단계별 처리 과정** (`NotificationSseService.sendNotification()`):

1. **연결 확인**
   - `emitters`에서 사용자 ID로 `SseEmitter` 조회
   - 연결되어 있지 않으면 로그만 기록하고 종료 (알림은 DB/Redis에 저장됨)

2. **SSE 이벤트 전송**
   - `SseEmitter.send()` 호출
   - 이벤트 이름: `"notification"`
   - 데이터: `NotificationDTO`

3. **전송 실패 처리**
   - `IOException` 발생 시:
     - `emitters`에서 사용자 제거
     - `emitter.completeWithError()` 호출하여 연결 종료
     - 로그 기록

**특징:**
- 연결된 사용자에게만 실시간 전송
- 연결이 없어도 알림은 DB/Redis에 저장 (나중에 조회 가능)
- 전송 실패 시 자동 정리

### 7. SSE 연결 해제 흐름

**단계별 처리 과정** (`NotificationSseService.removeConnection()`):

1. **연결 제거**
   - `emitters`에서 사용자 ID로 `SseEmitter` 제거

2. **연결 완료 처리**
   - `emitter.complete()` 호출하여 연결 정상 종료
   - 오류 발생 시 로그만 기록 (무시)

**특징:**
- 명시적 연결 해제 지원
- 연결 정리 및 리소스 해제

## 📊 데이터 흐름

### 1. 알림 생성 및 발송 흐름

```mermaid
sequenceDiagram
    participant Domain as 도메인 서비스<br/>(댓글 작성)
    participant NS as NotificationService
    participant MySQL as MySQL
    participant Redis as Redis
    participant NSS as NotificationSseService
    participant Client as 프론트엔드<br/>(SSE 클라이언트)
    
    Domain->>NS: createNotification(userId, type, title, content, ...)
    
    NS->>MySQL: INSERT INTO notifications
    MySQL-->>NS: 저장 완료
    
    NS->>NS: NotificationDTO 변환
    
    NS->>Redis: GET notification:{userId}
    Redis-->>NS: 기존 알림 목록 (또는 null)
    
    NS->>NS: 최신 알림을 맨 앞에 추가<br/>(최대 50개 유지)
    
    NS->>Redis: SET notification:{userId}<br/>(TTL: 24시간)
    Redis-->>NS: 저장 완료
    
    NS->>NSS: sendNotification(userId, dto)
    
    alt SSE 연결 존재
        NSS->>Client: SSE 이벤트 전송<br/>(event: notification)
        Client-->>Client: 알림 UI 업데이트
    else SSE 연결 없음
        NSS->>NSS: 로그만 기록<br/>(알림은 DB/Redis에 저장됨)
    end
    
    NS-->>Domain: NotificationDTO 반환
```

### 2. 알림 조회 및 병합 흐름

```mermaid
sequenceDiagram
    participant Client as 프론트엔드
    participant NC as NotificationController
    participant NS as NotificationService
    participant Redis as Redis
    participant MySQL as MySQL
    
    Client->>NC: GET /api/notifications?userId={userId}
    NC->>NS: getUserNotifications(userId)
    
    NS->>Redis: GET notification:{userId}
    Redis-->>NS: Redis 알림 목록 (또는 null)
    
    alt Redis에 데이터 있음
        NS->>MySQL: SELECT * FROM notifications<br/>WHERE user_idx = userId<br/>ORDER BY created_at DESC
        MySQL-->>NS: DB 알림 목록
        
        NS->>NS: mergeNotifications()<br/>- Redis ID 집합 생성<br/>- DB에서 Redis에 없는 알림만 추가<br/>- 최신순 정렬
        NS-->>NC: 병합된 알림 목록
    else Redis에 데이터 없음
        NS->>MySQL: SELECT * FROM notifications<br/>WHERE user_idx = userId<br/>ORDER BY created_at DESC
        MySQL-->>NS: DB 알림 목록
        NS-->>NC: DB 알림 목록
    end
    
    NC-->>Client: List<NotificationDTO>
```

### 3. SSE 실시간 알림 구독 흐름

```mermaid
sequenceDiagram
    participant Client as 프론트엔드<br/>(Navigation)
    participant SSE as SSE Endpoint
    participant NSS as NotificationSseService
    participant NS as NotificationService
    
    Client->>SSE: GET /api/notifications/stream?userId={userId}&token={token}
    SSE->>NSS: createConnection(userId)
    NSS->>NSS: SseEmitter 생성<br/>(타임아웃: 1시간)
    NSS->>NSS: emitters.put(userId, emitter)
    NSS-->>SSE: SseEmitter 반환
    
    SSE->>NS: getUnreadCount(userId)
    NS-->>SSE: 읽지 않은 알림 개수
    SSE->>Client: SSE 이벤트 전송<br/>(event: unreadCount)
    
    Note over Client,NSS: 연결 유지 (Keep-Alive)
    
    Note over NSS: 새 알림 생성 시
    NSS->>Client: SSE 이벤트 전송<br/>(event: notification, data: NotificationDTO)
    Client-->>Client: 알림 UI 업데이트<br/>읽지 않은 알림 개수 증가
```

### 4. 댓글 작성 시 알림 트리거 흐름

```mermaid
sequenceDiagram
    participant User as 사용자
    participant CS as CommentService<br/>/ CareRequestCommentService<br/>/ MissingPetCommentService
    participant NS as NotificationService
    participant Board as Board/CareRequest/MissingPetBoard
    
    User->>CS: 댓글 작성 요청
    CS->>CS: 댓글 저장
    
    CS->>Board: 게시글 작성자 조회
    Board-->>CS: 작성자 정보
    
    CS->>CS: 댓글 작성자 ≠ 게시글 작성자?<br/>(자기 자신에게 알림 방지)
    
    alt 작성자가 다름
        CS->>NS: createNotification(<br/>  boardOwnerId,<br/>  NotificationType.BOARD_COMMENT,<br/>  "내 게시글에 새로운 댓글이 달렸습니다",<br/>  "{작성자명}님이 댓글을 남겼습니다: {댓글 내용 (50자 제한)}",<br/>  boardId,<br/>  "BOARD"<br/>)
        
        Note over NS: 알림 생성 및 발송<br/>(DB 저장, Redis 저장, SSE 전송)
        
        NS-->>CS: 알림 생성 완료
    else 작성자가 같음
        CS->>CS: 알림 발송 안 함
    end
    
    CS-->>User: 댓글 작성 완료
```

## 🎯 핵심 설계 전략

### 1. 이중 저장소 전략 (Dual Storage Strategy)

**문제**: 실시간 알림 조회 성능과 데이터 안정성을 동시에 확보해야 함

**해결**: Redis와 MySQL을 함께 활용
- **Redis**: 최신 알림 50개를 24시간 TTL로 캐싱하여 실시간 조회 속도 극대화
- **MySQL**: 모든 알림을 영구 저장하여 이력 관리 및 안정성 보장
- **병합 전략**: Redis와 DB 데이터를 병합하여 중복 제거 후 최신순 정렬

**효과**:
- 빠른 조회 성능 (Redis 캐싱)
- 데이터 안정성 (MySQL 영구 저장)
- 완전한 데이터 제공 (병합 전략)

### 2. Redis 캐싱 전략

**Redis Key 구조**:
```
notification:{userId}
```

**저장 전략**:
- 최신 알림 50개만 저장 (메모리 효율성)
- TTL: 24시간 (최신 알림만 유지)
- 최신 알림을 리스트 **맨 앞**에 추가 (시간순 정렬과 함께 “최신 우선” 목록)

**효과**:
- 빠른 조회 성능
- 메모리 사용량 최적화
- 자동 만료로 관리 부담 감소

### 3. 병합 전략 (Merge Strategy)

**문제**: Redis와 DB에 중복 데이터가 있을 수 있음

**해결**: 중복 제거 및 정렬
- Redis 알림 ID 집합 생성
- DB에서 Redis에 없는 알림만 추가
- 최신순 정렬 (`createdAt` 기준 내림차순)

**효과**:
- 중복 없는 완전한 데이터 제공
- 최신순 정렬로 일관된 사용자 경험

### 4. SSE 실시간 알림 전략

**문제**: 사용자가 페이지를 새로고침하지 않아도 실시간으로 알림을 받아야 함

**해결**: SSE(Server-Sent Events) 활용
- 단방향 통신 (서버 → 클라이언트)
- HTTP 기반으로 방화벽 친화적
- 자동 재연결 지원
- `ConcurrentHashMap`으로 사용자별 연결 관리

**연결 관리**:
- 타임아웃: 1시간
- 연결 해제 시 자동 정리 (`onCompletion`, `onTimeout`, `onError`)
- 연결된 사용자에게만 실시간 전송

**효과**:
- 실시간 알림 푸시
- 서버 부하 최소화 (폴링 대비)
- 사용자 경험 향상

### 5. 자기 자신 알림 방지 전략

**문제**: 사용자가 자신의 게시글에 댓글을 달면 자신에게 알림이 가면 안 됨

**해결**: 댓글 작성자와 게시글 작성자 비교
- 댓글 작성자 ≠ 게시글 작성자인 경우에만 알림 발송
- 각 도메인 서비스에서 사전 체크

**효과**:
- 불필요한 알림 방지
- 사용자 경험 향상

### 6. 댓글 내용 길이 제한 전략

**문제**: 알림 내용에 너무 긴 댓글이 포함되면 UI가 깨질 수 있음

**해결**: 댓글 내용 50자 제한
- 50자 초과 시 `...` 추가
- 알림 생성 시점에 자동 처리

**효과**:
- 일관된 알림 UI
- 사용자 경험 향상

## 🔄 도메인 간 연동

### 1. Care 도메인 연동
- **트리거**: `CareRequestCommentService.addComment()`
- **알림 타입**: `CARE_REQUEST_COMMENT`
- **대상**: 펫케어 요청글 작성자
- **관련 타입**: `CARE_REQUEST`

### 2. Board 도메인 연동
- **트리거**: `CommentService.addComment()`
- **알림 타입**: `BOARD_COMMENT`
- **대상**: 커뮤니티 게시글 작성자
- **관련 타입**: `BOARD`

### 3. MissingPet 도메인 연동
- **트리거**: `MissingPetCommentService` (실종 제보 댓글 작성 시)
- **알림 타입**: `MISSING_PET_COMMENT`
- **대상**: 실종 제보 게시글 작성자
- **관련 타입**: `MISSING_PET` (`relatedId`는 게시글 `boardIdx` 등 도메인 로직에 맞게 전달)

## 📈 성능 최적화

### 1. Redis 최적화

#### 캐싱 전략
- **최신 50개만 저장**: 메모리 사용량 최적화
- **TTL 24시간**: 자동 만료로 관리 부담 감소
- **최신순 추가**: 빠른 조회 성능

#### Redis Key 설계
```
notification:{userId}
```
- 사용자별로 분리하여 조회 성능 향상
- 간단한 Key 구조로 관리 용이

### 2. DB 최적화

#### 인덱스 전략
```sql
-- 사용자별 알림 조회
CREATE INDEX idx_notification_user ON notifications(user_idx, created_at DESC);

-- 읽지 않은 알림 조회
CREATE INDEX idx_notification_user_read ON notifications(user_idx, is_read, created_at DESC);
```

**선정 이유**:
- 사용자별 조회가 빈번함
- 읽지 않은 알림 조회 최적화
- 최신순 정렬 최적화

### 3. 애플리케이션 레벨 최적화

#### 병합 최적화
- **Set 기반 중복 제거**: O(1) 조회 성능
- **스트림 처리**: 효율적인 필터링 및 정렬

#### SSE 연결 관리
- **ConcurrentHashMap**: 동시성 안전한 연결 관리
- **자동 정리**: 연결 해제 시 자동 제거

### 4. 프론트엔드 최적화

#### SSE 연결 관리
- **자동 재연결**: EventSource 자동 재연결 활용
- **폴백 폴링**: SSE 연결 실패 시 5분마다 폴링
- **중복 제거**: 알림 목록에 중복 추가 방지

#### 알림 UI 최적화
- **로컬 상태 관리**: 불필요한 API 호출 방지
- **읽지 않은 알림 개수 캐싱**: 실시간 업데이트

## 🔐 보안 고려사항

### 1. 권한 제어
- **REST 알림 API**: `@PreAuthorize("isAuthenticated()")` — JWT(헤더 또는 `token` 쿼리) 필요
- **목록/개수 API**: 쿼리 `userId`는 **클라이언트가 전달**; 현재 컨트롤러는 **JWT 주체와 `userId` 일치를 강제하지 않음**. 운영·보안상 본인 `Users.idx`만 사용할 것(서버 검증 강화는 선택 과제)
- **읽음 처리(`markAsRead`)**: `NotificationForbiddenException`으로 **알림 소유자만** 처리
- **SSE(`/api/notifications/stream`)**: 컨트롤러에 `@PreAuthorize` 없음 → **`JwtAuthenticationFilter`가 `token` 쿼리 또는 `Authorization`으로 인증** 후 `/api/**`의 `authenticated()` 규칙 적용

### 2. 데이터 검증
- **사용자 존재 확인**: 알림 생성 시 사용자 존재 여부 확인
- **자기 자신 알림 방지**: 댓글 작성자와 게시글 작성자 비교
- **댓글 내용 길이 제한**: 알림 내용에 50자 제한

### 3. 입력 검증
- SQL Injection 완화: JPA/파라미터 바인딩
- 알림 문구는 주로 서버에서 조합(댓글 일부 등) — 표시 시 프론트에서 안전하게 렌더링

## 📝 주요 API 엔드포인트

### 알림 목록 조회
```
GET /api/notifications?userId={userId}
→ List<NotificationDTO>
- Redis와 DB 병합하여 반환
- 최신순 정렬
```

### 읽지 않은 알림 목록 조회
```
GET /api/notifications/unread?userId={userId}
→ List<NotificationDTO>
- 읽지 않은 알림만 반환
```

### 읽지 않은 알림 개수 조회
```
GET /api/notifications/unread/count?userId={userId}
→ Long
- 읽지 않은 알림 개수만 반환
```

### 알림 읽음 처리
```
PUT /api/notifications/{notificationId}/read?userId={userId}
→ 200 OK (본문 없음)
- 본인의 알림만 읽음 처리 가능 (`notification.user.idx == userId`)
- Redis 캐시 목록에서 해당 ID 제거
```

### 모든 알림 읽음 처리
```
PUT /api/notifications/read-all?userId={userId}
→ 200 OK (본문 없음)
- 해당 사용자 미읽음 알림 일괄 읽음 + Redis 키 `notification:{userId}` 삭제
```

### SSE 실시간 알림 구독
```
GET /api/notifications/stream?userId={userId}&token={token}
→ text/event-stream
- SSE 연결 생성
- 연결 즉시 읽지 않은 알림 개수 전송
- 새 알림 생성 시 실시간 전송
```

## 🎯 핵심 포인트 요약

### 1. 이중 저장소 전략
- **Redis**: 최신 알림 50개, 24시간 TTL, 빠른 조회 성능
- **MySQL**: 모든 알림 영구 저장, 데이터 안정성
- **병합**: Redis와 DB 데이터 병합, 중복 제거, 최신순 정렬

### 2. 실시간 알림 푸시
- **SSE**: Server-Sent Events를 통한 실시간 알림 전송
- **연결 관리**: `ConcurrentHashMap` — **`userId`당 단일 emitter**(재연결 시 이전 연결 덮어씀)
- **자동 재연결**: EventSource 자동 재연결 지원
- **폴백 폴링**: SSE 연결 실패 시 폴링으로 대체

### 3. 다양한 알림 유형
- **CARE_REQUEST_COMMENT**: 펫케어 요청글 댓글 알림
- **BOARD_COMMENT**: 커뮤니티 게시글 댓글 알림
- **MISSING_PET_COMMENT**: 실종 제보 게시글 댓글 알림

### 4. 자기 자신 알림 방지
- **사전 체크**: 댓글 작성자와 게시글 작성자 비교
- **조건부 발송**: 작성자가 다른 경우에만 알림 발송

### 5. 성능 최적화
- **Redis 캐싱**: 최신 알림 50개만 저장
- **DB 인덱스**: 사용자별 조회, 읽지 않은 알림 조회 최적화
- **병합 최적화**: Set 기반 중복 제거, 스트림 처리

### 6. 사용자 경험
- **실시간 알림**: SSE를 통한 즉시 알림 전달
- **읽지 않은 알림 개수**: 실시간 업데이트
- **댓글 내용 제한**: 50자 제한으로 일관된 UI

...
Location 검색 keyword는 `LocationSearchPerformedEvent`로 추천 도메인에 전달된다(이벤트는 발행자 location 도메인 소유 — `domain/location/event`, 2026-07 리팩토링으로 `location→petRecommendation` 역방향 의존 제거). 추천 도메인은 intent signal을 만들고, 주변서비스 탭은 signal을 카드로 보여준다. 카드 선택은 Location `category` 필터 검색으로 연결된다.

...
sequenceDiagram
    ...
    alt RelatedType.CARE_REQUEST and CareRequest OPEN
        ...
        Chat->>Chat: requester/provider 제재 상태 재확인
...
sequenceDiagram
    ...
    loop each request
        alt requester 또는 ACCEPTED provider 제재 상태
            Sch->>Sch: 자동 완료 스킵, 관리자 검토 대상 로그
        else 제재 당사자 없음
...
제재 요청자의 기존 `OPEN` 케어는 `SUSPENDED`면 상태를 유지하고 공개 조회에서 제외하며, `BANNED`면 제재 이벤트 후 `CANCELLED`로 전환된다. 일반 사용자 단건 상세에서도 제재 요청자의 `OPEN`/`CANCELLED` 사전 매칭 요청은 비노출 처리한다.
...
- 거래 확정과 상태 변경은 요청자/제공자 제재 상태를 확인해 차단하지만, 관리자 상태 변경은 운영 판단으로 우회할 수 있다.

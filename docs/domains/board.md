...
- **효과**: 1000개 게시글 조회 시 2001개 쿼리 → 3개 쿼리로 감소
...
**설명**:
- **처리 흐름**: 댓글 저장 → commentCount 증가 → 알림 발송 (조건부)
...
| `getBoard()` | 게시글 상세 조회 | 조회수 증가 (중복 방지) |
| `createBoard()` | 게시글 생성 | 파일 첨부 처리, 캐시 무효화 |
| `updateBoard()` | 게시글 수정 | 파일 동기화, 캐시 무효화 |
| `deleteBoard()` | 게시글 삭제 (소프트 삭제) | 연관 댓글도 소프트 삭제 |
| `searchBoardsWithPaging()` | 게시글 검색 | 제목/내용/작성자 검색, FULLTEXT 인덱스 활용 |
...
| `addComment()` | 댓글 작성 | commentCount 증가, 알림 발송 |
| `deleteComment()` | 댓글 삭제 | 소프트 삭제, commentCount 감소 |

#### BoardPopularityService
| 메서드 | 설명 | 주요 로직 |
|--------|------|-----------|
| `getPopularBoards()` | 인기글 조회 | 스냅샷 조회, 없으면 생성 |
| `generateSnapshots()` | 인기글 스냅샷 생성 | 배치 조회로 실시간 집계, 상위 30개 저장 |
...
**적용 사례**:
- 반응 수 배치 조회: 500개 단위로 분할
- 인기글 스냅샷 생성: 1000개 단위로 분할
...
### 기술적 하이라이트
1. **N+1 문제 해결**: 배치 조회로 쿼리 수 667배 감소
2. **조회수 중복 방지**: BoardViewLog로 정확한 조회 수 추적
3. **인기글 스냅샷**: 미리 계산하여 조회 성능 30배 향상
4. **실시간 카운트 업데이트**: likeCount, commentCount 필드로 조회 성능 향상
5. **캐싱 전략**: Redis 캐싱으로 응답 시간 10배 향상

### 학습한 점
- N+1 문제 해결 방법 (배치 조회, Fetch Join)
- 조회수 중복 방지 전략 (별도 테이블 사용)
- 인기글 스냅샷 패턴 (미리 계산하여 저장)
- 실시간 카운트 업데이트 vs 집계 쿼리 트레이드오프
- 캐싱 전략 수립 및 무효화 전략

### 개선 가능한 부분
- Redis 조회수 관리: BoardViewLog 테이블 부담 감소
- ElasticSearch 검색: Full-Text Search 성능 향상
- 이미지 CDN: 첨부 이미지 로딩 속도 개선
- 분산 락: 반응 처리 동시성 제어 강화

...
4. `PaymentRecordedEvent` 발행 — 통계 집계는 트랜잭션 커밋 후(AFTER_COMMIT) 리스너가 처리
5. 에스크로 저장
...
- 에스크로 지급 완료 시 `PaymentRecordedEvent`를 발행하고, 통계 도메인 리스너가 커밋 후(AFTER_COMMIT, REQUIRES_NEW) `recordPayment(amount)`를 집계한다. 통계 실패가 코인 지급을 롤백하지 않도록 분리(2026-07).

...
| 인증 principal         | `backend/main/java/com/linkup/Petory/global/security/CustomUserDetails.java`                   |
| UserDetails 생성       | `backend/main/java/com/linkup/Petory/domain/user/service/UsersDetailsServiceImpl.java`         |
| 현재 사용자 idx 브리지 | `backend/main/java/com/linkup/Petory/global/security/AuthenticatedUserIdResolver.java`         |
...
로그인 이벤트를 append-only로 기록한다. DAU 정확한 집계를 위해 2026-06-28에 도입됐다.

| 필드          | 의미                                   |
| ------------- | -------------------------------------- |
| `id`          | PK                                     |
| `user`        | 로그인한 사용자 (`Users`, `LAZY`)      |
| `loginAt`     | 로그인 시각                            |
| `loginMethod` | `LOCAL` / `GOOGLE` / `NAVER` / `KAKAO` |

인덱스: `(user_id, login_at)`, `(login_at)` — 통계 집계 쿼리 최적화용.

`AuthService.login()` (로컬) 및 `OAuth2Service.processOAuth2Login()` (소셜) 두 진입점에서 행을 삽입한다.
삭제·수정 없이 append만 한다.
...
- refresh 시점에도 사용자 제재 상태를 다시 확인한다. `BANNED` 또는 만료 전 `SUSPENDED`면 refresh token을 제거하고 재발급을 거부한다.
- 만료된 `SUSPENDED` 사용자가 refresh를 요청하면 `ACTIVE`로 자동 전환한 뒤 access token을 재발급한다.

### 보호 API 인증 주체 흐름

Access Token으로 보호 API를 호출하면 `JwtAuthenticationFilter`가 JWT를 검증하고 `UsersDetailsServiceImpl.loadUserByUsername(subject)`를 호출한다. 이때 subject는 `Users.id`이고, `UsersDetailsServiceImpl`은 active user를 조회한 뒤 Spring 기본 `User`가 아니라 `CustomUserDetails`를 principal로 만든다.
...
`CustomUserDetails`에 담기는 값:

| 값              | 용도                                            |
| --------------- | ----------------------------------------------- |
| `idx`           | DB FK, 현재 사용자 PK                           |
| `loginId`       | JWT subject, `UserDetails#getUsername()` 반환값 |
| `role`          | `ROLE_USER`, `ROLE_ADMIN` 등 권한 생성          |
| `emailVerified` | 기능 사용 자격 검사에 활용 가능                 |
| `status`        | `isEnabled`, `isAccountNonLocked` 판단          |

현재 적용 상태:

- `UserProfileController`, `PetController`는 `@AuthenticationPrincipal CustomUserDetails`를 직접 받는다.
- 여러 기존 컨트롤러는 아직 `AuthenticatedUserIdResolver.requireCurrentUserIdx()`를 사용한다.
- resolver는 principal이 `CustomUserDetails`면 DB 재조회 없이 `idx`를 반환하고, 그렇지 않으면 이전 방식대로 loginId로 active user를 조회한다.
- `UsersDetailsServiceImpl`은 만료된 `SUSPENDED` 사용자를 로드하면 `ACTIVE`로 전환한 뒤 `CustomUserDetails`를 만든다.
- `JwtAuthenticationFilter`는 `BANNED`를 항상 거부하고, 만료 전 `SUSPENDED`는 `POST /api/reports`만 예외 허용한다. 만료된 `SUSPENDED`는 `suspendedUntil` 기준으로 보호 API 인증을 허용한다.
- 제재로 거부되는 access token 요청은 인증 실패 401로 흘리지 않고 403 JSON 응답을 반환한다.
- WebSocket handshake와 STOMP channel 인증도 같은 계정 사용 가능 여부 검사를 수행한다.
- `AuthController.validate`, `AuthController.logout`은 아직 Authorization header를 직접 파싱한다.
- `JwtAuthenticationFilter`는 현재 `validateToken`, `isTokenExpired`, `getIdFromToken`을 각각 호출하므로 JWT claims 1회 파싱 구조로는 아직 정리되지 않았다.
...
- `PetController`는 `@AuthenticationPrincipal CustomUserDetails`에서 `loginId`를 꺼내 서비스로 전달한다.
- 상세/수정/삭제/복구는 현재 principal의 `loginId`와 펫 소유자의 `Users.id`가 같아야 한다.
...
- 신고 처리의 `WARN_USER`, `SUSPEND_USER` 액션은 `UserSanctionService.applySanctionFromReport()`로 연결된다. 현재 `SUSPEND_USER`는 3일 `SUSPENSION`으로 처리된다.
- 신고 대상이 게시글/댓글/실종 게시글/케어 리뷰이면 `ReportService`가 콘텐츠 ID를 작성자 user idx로 변환한 뒤 제재를 적용한다.
...
- `ReportServiceTest`: 게시글/댓글 신고 제재 대상이 콘텐츠 ID가 아니라 작성자 user idx로 resolve되는지 확인
...
- JWT 보호 API의 principal은 `CustomUserDetails`로 통일되어 `idx`, `loginId`, `role`, `emailVerified`, `status`를 담는다.
- `AuthenticatedUserIdResolver`는 `CustomUserDetails` principal이면 현재 사용자 idx를 DB 재조회 없이 반환한다.

남은 주의점:

- `JwtAuthenticationFilter`의 JWT 검증/만료확인/subject 추출은 아직 claims를 여러 번 파싱한다.
- `AuthController.validateToken()`과 `logout()`은 아직 Authorization header를 직접 파싱한다.
...
- [인증 주체 조회 리팩터링 점검](../refactoring/authentication-principal-refactoring.md)

...
### 5.1 문제 1: 읽지 않은 메시지 수 동시 증가 문제

#### 문제 상황
- **발생 시점**: 여러 메시지가 동시에 전송될 때
- **증상**: `unreadCount`가 부정확하게 증가
- **영향 범위**: 읽지 않은 메시지 수 부정확

#### 원인 분석
- **근본 원인**: 애플리케이션 레벨에서 `unreadCount` 증가 시 Lost Update 발생
- **재현 방법**: 동시에 여러 메시지 전송
...
@Query("UPDATE ConversationParticipant cp " +
       ...
       "WHERE cp.conversation.idx = :conversationIdx " +
       "  AND cp.user.idx != :senderIdx " +
...
#### 결과
- **개선 효과**: 읽지 않은 메시지 수 정확도 100% 달성
- **성능 향상**: Lost Update 문제 해결

### 5.2 문제 2: 채팅방 목록 조회 N+1 문제

#### 문제 상황
- **발생 시점**: 채팅방 목록 조회 시 각 채팅방의 참여자, 마지막 메시지를 개별 쿼리로 조회
- **증상**: 채팅방이 많을수록 쿼리 수 증가
- **영향 범위**: 목록 조회 성능 저하

#### 원인 분석
- **근본 원인**: 채팅방 목록 조회 후 각 채팅방의 연관 데이터를 개별 쿼리로 조회
- **재현 방법**: 채팅방 목록 조회 시 로그 확인

#### 해결 방법
```java
// Before: N+1 문제
List<Conversation> conversations = conversationRepository.findActiveConversationsByUser(userId);
for (Conversation conv : conversations) {
    ConversationParticipant myParticipant = participantRepository
            .findByConversationIdxAndUserIdx(conv.getIdx(), userId);  // 개별 쿼리
    List<ConversationParticipant> participants = participantRepository
            .findByConversationIdxAndStatus(conv.getIdx(), ParticipantStatus.ACTIVE);  // 개별 쿼리
}

// After: Fetch Join + 배치 조회
@Query("SELECT DISTINCT c FROM Conversation c " +
       "INNER JOIN FETCH c.participants p " +
...
List<Conversation> findActiveConversationsByUserWithParticipants(
...
#### 결과
- **개선 효과**: N+1 쿼리 → 2개 쿼리로 감소
- **성능 향상**: 목록 조회 응답 시간 60% 감소

### 5.3 문제 3: WebSocket 연결 누수

#### 문제 상황
- **발생 시점**: WebSocket 연결 종료 시 연결 리소스 정리 누락
- **증상**: 메모리 사용량 증가, 연결 누수
- **영향 범위**: 서버 리소스 부족

#### 원인 분석
- **근본 원인**: WebSocket 연결 종료 시 세션 정리 로직 누락
- **재현 방법**: WebSocket 연결 후 연결 종료 시 리소스 확인

#### 해결 방법
```java
// After: WebSocket 연결 종료 이벤트 처리
@Component
public class WebSocketEventListener {
    ...
    public void handleWebSocketDisconnectListener(SessionDisconnectEvent event) {
        ...
        if (username != null) {
            ...
            // 연결 종료 시 필요한 정리 작업 수행
            // 예: 사용자 온라인 상태 업데이트, 채팅방에서 나가기 등
        }
    }
}
```

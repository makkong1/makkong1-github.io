...
거래 확정 전에는 현재 클릭한 사용자뿐 아니라 채팅방의 ACTIVE 참여자 전체를 `isSanctioned()` 기준으로 다시 확인한다. 실제 `CareRequest` 처리 직전에도 requester/provider를 다시 조회해 둘 중 하나라도 제재 상태이면 `IN_PROGRESS` 전환과 에스크로 생성을 진행하지 않는다.
...
> 코드 기준: `ChatMessageService`, `ConversationService`, `ConversationDTO`, `ChatForbiddenException`

### 메시지 전송 차단

`ChatMessageService.sendMessage()` 진입 시 발신자(`sender`)가 `isSanctioned()` 상태이면 `ChatForbiddenException.sanctionedUserCannotSend()` (403)을 던진다. REST 전송과 STOMP WebSocket 전송이 모두 이 서비스를 통하므로 통합 차단된다.

### 거래 확정 차단

`ConversationService.confirmCareDeal()`은 아래 조건 중 하나라도 만족하면 `ChatForbiddenException.sanctionedPartyCannotConfirmDeal()` (403)을 던진다.

- 현재 거래 확정을 누른 사용자가 제재 상태
- 채팅방의 ACTIVE 참여자 중 제재 사용자가 있음
- 실제 케어 요청 requester 또는 provider가 제재 상태

### 채팅방 DTO 안내 플래그

`ConversationDTO`에 `hasSanctionedParticipant: boolean` 필드가 추가되었다.

- 채팅방 목록 조회 (`GET /api/conversations`): 활성(`ACTIVE`) 참여자 중 `isSanctioned()` 상태가 하나라도 있으면 `true`.
- 채팅방 상세 조회 (`GET /api/conversations/{id}`): 동일 기준.
- 프론트엔드 안내 UI 표시용 플래그이며, 거래 확정 버튼 비활성화 등에 활용할 수 있다.

...
| **profile** | 프로필 페이지 집계(User + Care/Location 리뷰 + Meetup 이력 조합; **집계 계층**) | 컨트롤러·DTO 중심 (`UserProfileController`, `UserProfileWithReviewsDTO`) |
...
- **profile**은 user·care·location·meetup을 조합해 프로필 페이지를 그리는 **집계 계층**입니다(2026-07 신설). `user`가 타 도메인을 역방향 의존하지 않도록 `UserProfileController`·`UserProfileWithReviewsDTO`를 여기로 옮겼습니다. → `docs/analysis/domain-dependency-refactoring-2026-07.md`

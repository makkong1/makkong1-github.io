...
**`FacilitySyncScheduler`**: `@Scheduled(cron = "0 0 1 * * *")` 매일 01:00. `app.location.import.file-path`가 비어있으면 skip.
...
- 지역 계층(`sido/sigungu/eupmyeondong/road_name`) + `is_deleted` + `rating DESC` 복합 인덱스 → `findBy*` 쿼리의 WHERE·ORDER BY 커버, `USE INDEX` 힌트로 확정 사용
...
- **인덱스 전략**: 지역 계층 복합 인덱스(`sido/sigungu/eupmyeondong/road_name + is_deleted + rating`)·FULLTEXT·공간 인덱스로 조회 성능 확보; `USE INDEX` 힌트로 옵티마이저 오선택 방지

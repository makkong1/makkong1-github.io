...
3. **3단계 (After 2)**: Bounding Box 적용 - `(latitude, longitude)` B+Tree 인덱스로 스캔 범위 축소

> **4단계 (현행)**: 공간 인덱스(R-Tree) 전환 — `geo_point` POINT 컬럼 + `ST_Within`/`ST_Distance_Sphere`.
> 아래 성능 비교표는 **1~3단계 실측치**이며, 4단계는 3단계 측정 환경 소실로 동일 조건 실측이 없다.
> 4단계 전환의 구조적 선택 근거는 [인덱스 분석 문서 §4단계](./index-analysis.md#-4단계-현행-공간-인덱스r-tree로-전환) 참조.

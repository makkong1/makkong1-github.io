...
| 3        | Location 검색/초기 로드 최적화 | 사용자 체감 성능, DB 필터링, 네트워크 전송량 감소를 말할 수 있다.               |
...
## 본문 사례 3: Location 검색/초기 로드 최적화

### 포트폴리오 문구

위치 서비스 초기 로드와 검색 전략을 두 단계로 개선했다. 1단계에서는 초기 로드 시 약 22,699개 전체 데이터를 전송하고 프론트엔드에서 필터링하던 구조를 위치 기반 10km 반경 조회로 바꿔, 조회 데이터를 약 1,026개로 줄이고 프론트 처리 시간을 1,484ms에서 약 700ms로 낮췄다. 2단계에서는 지역명 검색어가 들어온 경우 좌표 반경 검색 대신 `sido`/`sigungu` 파라미터로 지역 검색을 수행하는 우회 경로를 추가했다. 이 경로는 시군구 검색 기준 36~53ms 수준으로 측정됐다.
...
원인:
초기 로드는 검색 조건을 DB WHERE로 충분히 밀어 넣지 못했고, 프론트엔드에서 지역 필터링과 거리 계산을 수행했다. 현재 통합 지도 기본 조회와 "이 지역" 버튼은 좌표+반경 검색을 사용하지만, 지역명 검색어는 `sido`/`sigungu` 직접 검색으로 우회한다.

해결:
1단계로 사용자 위치 기준 10km 반경 조회를 백엔드에서 수행해 전체 데이터 전송을 줄였다. 2단계로 프론트에서 지역명 검색어를 감지하면 좌표 변환/반경 검색 대신 `sido` 또는 `sigungu`를 넘겨 DB 인덱스 기반 지역 검색을 타도록 했다.
...
- [Location 검색 전략 비교](../troubleshooting/location/search-strategy-comparison.md)
...
- 해결: 1단계 위치 기반 반경 조회, 2단계 지역명 검색어는 시군구 검색으로 우회
- 결과: 초기 조회 데이터 22,699개 -> 약 1,026개, 지역명 검색 시 시군구 쿼리 36~53ms
...
3. Location은 "1단계 전체 조회 제거, 2단계 지역명 검색어는 시군구 검색으로 우회"로 설명한다. 통합 지도 기본 조회와 이 지역 버튼은 현재도 좌표+반경 검색이다.

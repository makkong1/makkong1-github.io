...
Map<Long, List<FileDTO>> files =
  ...
  },
  {
    ...
    decision:
      '① users 상태를 board 쪽 author_visible 컬럼으로 비정규화(트리거로 동기화)해 조인 제거 → ② 커버링 인덱스로 idx만 훑어 깊은 위치까지 skip → ③ 살아남은 20건만 작성자 조인, COUNT는 board 단일 테이블. 키셋 페이징도 검토했으나 앱 전역이 페이지 번호·점프 UI(공유 PageNavigation)를 써서 못 채택했다.',
    verify:
...
SELECT ... FROM board b JOIN users u ON ...
 ...
 ORDER BY b.created_at DESC`,
  },
  {
    ...
    decision:
      '"동시성 = 무조건 락"이 아니라 연산 성격으로 전략을 나눔. 모임 인원(단순 조건부 증가)은 원자적 UPDATE 한 문장(WHERE current < max) — 락·데드락 관리가 필요 없어서 택함(속도가 아니라 성격 기준). 펫코인 차감(잔액 부족 검증 필요)은 현재값을 읽고 분기해야 해 비관적 락. 케어 확정(두 참여자 상태를 함께 판단)은 원자적 UPDATE로 표현 불가라 상위 엔티티(Conversation) 락으로 직렬화.',
    verify:

...
function LocationDomainV2() {
  ...
  const flowDiagram = `flowchart TD
    ...
    subgraph FE["프론트엔드"]
        MV["mapViewportCenter\\n지도 뷰포트 중심"]
        SC["searchCenter\\n실제 검색 기준점"]
        BTN["이 지역 검색 버튼\\ncommitLocationSearch"]
    end

    subgraph API["LocationServiceController"]
        BR["반경 검색\\nlat·lng·radius"]
        RG["지역 검색\\nsido·sigungu 등"]
        KW["FULLTEXT 검색\\nkeyword 단독"]
        AL["전체 평점순"]
    ...
    subgraph DB["DB / Repository"]
        SP["ST\\_Within\\n+ ST\\_Distance\\_Sphere"]
        IDX["지역 인덱스\\nidx\\_sigungu\\_deleted\\_rating"]
    ...
    <div className="domain-page-wrapper" style={{ padding: '2rem 0' }}>
      ...
      >
        <div className="domain-page-content" style={{ flex: 1 }}>
          ...
          >
            Location 도메인은 반려동물 관련 병원, 카페, 공원, 펫샵 같은 장소를
            위치·지역·키워드 기준으로 탐색하는 기능입니다. 단순 지도 API
            연동보다 검색 기준의 일관성과 초기 로드 성능을 더 중요한 문제로
            다뤘습니다. 특히 지도 이동 자체가 곧바로 결과를 바꾸지 않도록{' '}
            <code
              style={{
                backgroundColor: 'var(--bg-color)',
            ...
            를 분리하고, 입력 조건에 따라 검색 경로를 명시적으로 분기했습니다.
          </p>
          ...
          >
            ...
            <Card style={{ marginBottom: '1rem' }}>
              ...
              >
                검색 로직을 위치 우선, 지역 우선, 키워드 단독 FULLTEXT로
                명확히 분기하고, 초기 로드에서는 사용자 주변 데이터를 줄여
                가져오도록 구조를 바꿨습니다. 메인 지도에서는{' '}
                <code
                  style={{
                    backgroundColor: 'var(--bg-color)',
                    padding: '0.1rem 0.3rem',
...
if (hasLocation)      results = searchByLocation(...);
else if (hasRegion)   results = searchByRegion(...);
else if (hasKeyword)  results = searchByKeyword(...);   // FULLTEXT
else                  results = searchByRegion(null, null, null, null, ...);`}</CodeBlock>
            </Card>
            ...
            <Card style={{ marginBottom: '1rem' }}>
              ...
              >
                {li('이전: size 제한 없이 전체 22,699건 로드 → 전송량·메모리 병목')}
                {li('이후: 사용자 위치 기반 반경 검색 + 줌 레벨별 size 30~500 제한')}
...
size: getLimitForLevel('location', mapLevel)`}</CodeBlock>
            ...
            <Card style={{ marginBottom: '1rem' }}>
              ...
              >
                ...
                {li('"이 지역 검색" 클릭 → commitLocationSearch(mapViewportCenter) → searchCenter 확정 → 반경 검색 재실행')}
                {li('두 값이 다르면 "이 지역 검색" 버튼 표시 → 사용자가 재조회 시점을 직접 선택')}
              </ul>
...
const handleSearchThisArea = () =>
  commitLocationSearch(mapViewportCenter);`}</CodeBlock>
            ...
            <Card style={{ marginBottom: '1rem' }}>
              ...
              >
                {li('반경 검색: ST_Within 공간 조건 + keyword(LIKE) + category + sort')}
                {li('지역 검색: idx_sigungu_deleted_rating 인덱스 + keyword(LIKE) + category')}
                {li('키워드 단독: MATCH(name, description, category1~3) FULLTEXT — 반경/지역 없을 때만')}
                {li('클라이언트가 재필터링하지 않고 서버 SQL에서 일관되게 처리')}
              </ul>
              <CodeBlock>{`-- 반경 검색: MBR bbox pre-filter → ST_Distance_Sphere 정밀 필터 → LIKE keyword
...
-- FULLTEXT(ft_search)는 keyword 단독 경로(위치·지역 없을 때)에서만 사용`}</CodeBlock>
            ...
            <Card>
              ...
              >
                {li('우선순위: roadName → eupmyeondong → sigungu → sido → 전체')}
                {li('각 계층별 인덱스 활용 — 반경 검색 대비 응답 속도 우위')}
                {li('현재 메인 UI는 lat/lng 반경 검색 경로 사용 — 지역 검색은 추후 확장 경로')}
              </ul>
